...
main_recurse2Tramp = { world, count ->
    ...
    main_recurse2Tramp.trampoline(progress2(world), count + 1)
}.trampoline()
*/
...
def main_recurse1(world, count) {
    ...
    minY = min_y world
    maxY = max_y world
    println to_string2For(world)
    println "^ ${count}-----------------------------------(${minX},${minY})-(${maxX},${maxY}) = ${(maxX-minX)*(maxY-minY)}"
    main_recurse1(progress2For(world), count + 1)
}
